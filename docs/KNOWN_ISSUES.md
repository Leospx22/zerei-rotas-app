...
| ZR-F007 | Standalone native map requires a valid Google Maps SDK key in the EAS build environment. | Monitoring | Unassigned | `GOOGLE_MAPS_API_KEY` must be configured outside Git and restricted to `com.zereirotas.app` plus the active signing SHA-1. Rebuild the APK after key or manifest changes. |
